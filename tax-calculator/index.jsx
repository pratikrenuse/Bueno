...
// Country list: code/flag/isEUEEA stay constant; display names come from i18n.
const COUNTRIES = [
...
// Map property-use codes to their i18n key suffixes.
const USE_KEY = { personal: 'personal', short_rental: 'short', long_rental: 'long', mixed: 'mixed' };
const FILING_KEY = { always: 'always', missed_some: 'missed', never: 'never', unsure: 'unsure' };
...
export default function TaxCalculator() {
  ...
  const propertyUseTitle = (code) => tt(`use_${USE_KEY[code]}_t`);
  const propertyUseDesc  = (code) => tt(`use_${USE_KEY[code]}_d`);
  const filingTitle      = (code) => tt(`filing_${FILING_KEY[code]}_t`);
  const filingDesc       = (code) => tt(`filing_${FILING_KEY[code]}_d`);
  ...
  const resultHeadline = () => {
    if (!results) return '';
    if (results.yearsUnfiled === 0) return tt('result_headline_current');
    if (results.yearsUnfiled === 1) return tt('result_headline_unfiled_one');
    return tt('result_headline_unfiled_many').replace('{n}', results.yearsUnfiled);
  };
    ...
    <div className="calc-shell">
        ...
        <div className="intro-hero">
          <div className="intro-left">
            <div className="intro-hero-inner">
              ...
              <h1 className="intro-headline">{tt('intro_headline_1')}<br />{tt('intro_headline_2')}<br />{tt('intro_headline_3')} <em>{tt('intro_headline_em')}</em></h1>
          ...
          <div className="option-stack">
            {PROPERTY_USE_CODES.map(code => (
              <button key={code} className={`option-card ${form.propertyUse === code ? 'selected' : ''}`} onClick={() => selectPropertyUse(code)}>
                <div><p className="option-title">{propertyUseTitle(code)}</p><p className="option-desc">{propertyUseDesc(code)}</p></div>
              </button>
            ))}
          </div>
        </div></div>
          ...
          <div className="option-stack">
            {FILING_CODES.map(code => (
              <button key={code} className={`option-card ${form.filingHistory === code ? 'selected' : ''}`} onClick={() => selectFiling(code)}>
                <div><p className="option-title">{filingTitle(code)}</p><p className="option-desc">{filingDesc(code)}</p></div>
              </button>
            ))}
          </div>
        </div></div>
      ...
      {step === 'results' && results && (
        <div className="results-screen"><div className="results-inner">
          ...
          <p className="results-subline">{tt('result_subline').replace('{country}', form.countryName).replace('{use}', propertyUseTitle(form.propertyUse))}</p>
          ...
          <BuenoCTA variant="tax" href="https://getbueno.com" />
