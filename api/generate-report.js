export default async function handler(req, res) {
  ...
  if (!taxData) {
    return res.status(400).json({ error: 'Tax data required' });
  }
...
NEVER use em dashes, emojis, bullet points, or words like "revolutionary", "crucial", or "game-changing".
...
- Annual tax obligation: €${Number(taxData.annualTax).toFixed(0)}
- Filing history: ${filingLabel}
- Years potentially unfiled: ${taxData.yearsUnfiled}
- Total potential liability including outstanding years: €${Number(taxData.totalLiability).toFixed(0)}

Paragraph 1: Explain their specific tax situation in plain terms (what they owe and why, based on their country and property use).
Paragraph 2: Address their filing status honestly. If they have unfiled years, be direct but calm about what that means.
